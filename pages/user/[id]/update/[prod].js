...
export default function updateProduct() {
  ...
  const onSubmit = async (e) => {
    try {
      if (files) {
        ...
        const response = await baseURL.put(`/api/product/${router.query.prod}`, formData);
  ...
  const getProductDetails = async () => {
    try {
      const response = await baseURL.get(`api/product/${router.query.prod}`);
      ...
      if (response.data.status === 200) {
        ...
        response.data.data.images.map((img) => {
          urls.push({
            url: `${baseURL.defaults.baseURL}/${img.data}`,
          });
        });
        ...
        <section className="flex items-center min-h-screen justify-center font-poppins">
          <div className="mx-auto w-full flex flex-col 2lg:flex-row items-center mt-20 2lg:mt-0">
            ...
            <div className="w-full mx-auto px-6 py-4 rounded-md bg-gray-50/80">
              <div>
                <h2 className="text-center text-xl font-bold tracking-wide text-gray-800">Update Produk</h2>
              </div>
